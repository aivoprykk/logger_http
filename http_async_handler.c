...
static int http_config_set_value(const char *name, const char *json_value,
								 bool commit) {
	...
	if (json_value_pointer > json_value) {
		while (json_value_pointer < end_ptr) {
			if (json_value_pointer && (*(json_value_pointer - 1) == ':' &&
									   *(json_value_pointer - 2) == '"' &&
									   *(json_value_pointer - 3) == 'e' &&
									   *(json_value_pointer - 4) == 'u' &&
									   *(json_value_pointer - 5) == 'l'))
				break; // just on value
			++json_value_pointer;
		}
		is_json = true;
	...
	if (config_set_item_by_name(name,
								is_json ? json_value_pointer : json_value)) {
		err = ESP_OK;
	}
	if (err == ESP_OK) {
...
static int http_config_set_bulk(const char *json, bool commit) {
	...
	while (current < obj_end) {
		...
		// Skip to :
		while (current < obj_end && (*current == ' ' || *current == ':'))
			current++;
		...
		if (http_config_set_value(key, value, false) != 0)
			return -1;
	}
...
#else
static inline esp_err_t queue_request(httpd_req_t *req,
...
static esp_err_t set_content_type_from_file(httpd_req_t *req,
	...
	for (uint8_t
			 i = 0,
			 j = sizeof(http_file_extensions) / sizeof(http_file_extensions[0]),
			 k = 0;
		 i < j; ++i) {
...
static esp_err_t http_send_json_msg(httpd_req_t *req, const char *msg,
									int msg_size, int status, char *data,
									int data_size) {
...
static esp_err_t archive_file(httpd_req_t *req, const char *filename,
							  const char *base) {
	FUNC_ENTRY(TAG);
	int ret = ESP_OK;
	strbf_t sb;
	...
	if (!s_xfile_exists(strbf_finish(&sb))) {
		...
		if (ret < 0) {
			ELOG(TAG, "Failed to mkdir %s (%s)", sb.start,
				 esp_err_to_name(ret));
			goto done;
		}
	}
...
static esp_err_t send_file(httpd_req_t *req, int fd, uint32_t len) {
	...
	IMEAS_END_ARGS(TAG, " sent %" PRIu32 " bytes", len - i);
	return ret;
}

static esp_err_t config_handler(httpd_req_t *req, const char *name, strbf_t *sb,
								size_t flush_size) {
	...
	if (name && *name) {
		...
		for (size_t i = 0; i < module_count; ++i) {
			...
			if (reg->flags & CONFIG_MODULE_FLAG_SUBMODULE)
				continue;
			const size_t size = reg->ops.item_count;
			bool firstingroup = true;
			strbf_puts(sb, firstingroups ? "" : ",");
			firstingroups = false;
			strbf_puts(sb, "{\"group_id\":");
			strbf_putn(sb, reg->handle);
			strbf_puts(sb, ",\"default_hidden\":");
			strbf_puts(sb, (reg->flags & CONFIG_MODULE_FLAG_DEFAULT_HIDDEN)
							   ? "1"
							   : "0");
			strbf_puts(sb, ",\"group_name\":\"");
			strbf_puts(sb, reg->name);
			strbf_puts(sb, "\",\"items\":[");
			for (size_t index = 0; index < size; ++index) {
				if (!firstingroup)
					strbf_putc(sb, ',');
				firstingroup = false;
				config_manager_get_item_by_group_idx(reg->handle, index, sb);
				if (*(sb->cur - 1) == ',') {
					sb->cur--;
				}
	...
	DLOG(TAG, "groups sent, final flush at %zu bytes", sb->cur - sb->start);
	if (sb->cur > sb->start) {
...
static int uint8_array_to_ipv4_string(uint8_t *ipv4, char *buf) {
	return sprintf(buf, "%" PRIu8 ".%" PRIu8 ".%" PRIu8 ".%" PRIu8 "", ipv4[0],
				   ipv4[1], ipv4[2], ipv4[3]);
}

static void flush_d(httpd_req_t *req, const char *str, strbf_t *data,
					size_t flush_size) {
...
static esp_err_t paths_handler(httpd_req_t *req, data_mode_t mode,
							   strbf_t *data, size_t flush_size) {
	FUNC_ENTRY_ARGS(TAG, "%s", req->uri);
	// IMEAS_START();
	DIR *dirp = NULL;
	...
	while (i < VFS_MAX_PARTS) {
		if (vfs_ctx.parts[i].is_mounted) {
			...
			if (mode == DATA_MODE_JSON)
				...
				if (mode == DATA_MODE_HTML)
					flush_d(req, http_async_handler_strings[8], data,
							flush_size);
				flush_d(req, "Store", data, flush_size);
				if (mode == DATA_MODE_HTML)
					flush_d(req, http_async_handler_strings[9], data,
							flush_size);
				else
...
static esp_err_t system_info_get_handler(httpd_req_t *req, data_mode_t mode,
										 strbf_t *data, size_t flush_size) {
	FUNC_ENTRY(TAG);
	char lbuf[16] = {0};
	size_t llen = 0;
	...
	if (chip_info.model != 1)
		flush_d(req, "s3", data, flush_size);
	if (mode == DATA_MODE_HTML) {
		flush_d(req, http_async_handler_strings[8], data, flush_size);
	...
	if (mode == DATA_MODE_HTML) {
		...
		flush_d(req, ",\"totalheap\":", data, flush_size);
	...
	if (mode == DATA_MODE_HTML) {
		...
		flush_d(req, ",\"version\":\"", data, flush_size);
	...
	if (wifi_is_ap_ready()) {
		...
		if (mode == DATA_MODE_HTML) {
			...
			flush_d(req, "\",\"ap_address\":\"", data, flush_size);
		llen =
			uint8_array_to_ipv4_string(wifi_context.ap.ipv4_address, &lbuf[0]);
		flush_d(req, lbuf, data, flush_size);
		if (mode != DATA_MODE_HTML) {
			flush_d(req, "\"", data, flush_size);
		}
	...
	if (wifi_is_sta_connecting()) {
		...
		if (wifi_is_sta_connected())
			flush_d(req, wifi_context.stas[wifi_context.s_sta_num_connect].ssid,
					data, flush_size);
		if (mode == DATA_MODE_HTML) {
			flush_d(req, http_async_handler_strings[8], data, flush_size);
			...
			flush_d(req, "\",\"sta_address\":\"", data, flush_size);
		if (wifi_is_sta_connected())
			llen = uint8_array_to_ipv4_string(
				wifi_context.stas[wifi_context.s_sta_num_connect].ipv4_address,
				&lbuf[0]);
		flush_d(req, lbuf, data, flush_size);
		if (mode != DATA_MODE_HTML) {
			flush_d(req, "\"", data, flush_size);
...
off_t file_handler(httpd_req_t *req, strbf_t *path, size_t len,
				   const char *name, strbf_t *data, size_t flush_size,
				   uint32_t idx, data_mode_t mode) {
	struct tm *tm_info;
	struct stat sb;
	int statok;
	off_t ret = ESP_OK;
	...
	*path->cur = 0;
	...
	if ((sb.st_mode & S_IFMT) == S_IFDIR)
		type = 'd';
	else if (strstr(name, "config"))
		type = 'c';
	else
		type = 'f';
	if (mode == DATA_MODE_HTML) {
		if (type == 'f')
			flush_d(req, "<td><input type=\"checkbox\"></td>", data,
					flush_size);
		else
	...
	if (mode == DATA_MODE_HTML) {
		...
		flush_d(req, "\",\"date\":\"", data, flush_size);
	else
		strbf_putc(data, ' ');
	if (!statok)
		flush_d(req, tbuffer, data, flush_size);
	if (mode == DATA_MODE_HTML)
		flush_d(req, http_async_handler_strings[9], data, flush_size);
	else if (mode == DATA_MODE_JSON)
		flush_d(req, "\",\"size\":\"", data, flush_size);
	else
		strbf_putc(data, ' ');
	...
	if (mode == DATA_MODE_HTML)
		flush_d(req, "</td><td class=\"hide-xs\">", data, flush_size);
	else if (mode == DATA_MODE_JSON)
		flush_d(req, "\",\"type\":\"", data, flush_size);
	else
		strbf_putc(data, ' ');
	...
	if (mode == DATA_MODE_HTML)
		...
		flush_d(req, "\",\"mode\":\"", data, flush_size);
	else
		strbf_putc(data, ' ');
	if (type == 'c')
		strbf_putc(data, 'r');
	else
		flush_d(req, "rw", data, flush_size);
	if (mode == DATA_MODE_HTML) {
		flush_d(req, "</td>", data, flush_size);
		if (type == 'f') {
			flush_d(req, "<td><div role=\"group\" data-file=\"", data,
					flush_size);
			flush_d(req, name, data, flush_size);
			flush_d(req,
					"\"><button class=\"outline rm\">Delete</button><button "
					"class=\"outline ar\">Archive</button></div></td>",
					data, flush_size);
		} else {
			flush_d(req, "<td></td>", data, flush_size);
		}
...
static esp_err_t directory_handler(httpd_req_t *req, const char *path,
								   const char *match, data_mode_t mode,
								   strbf_t *data, size_t flush_size) {
	...
	if (!dirp) {
		httpd_resp_set_status(req, HTTPD_500);
		if (mode == DATA_MODE_HTML)
			httpd_resp_send_chunk(req, "<div>Error opening directory</div>",
								  35);
		else if (mode == DATA_MODE_JSON)
	...
	if (mode == DATA_MODE_HTML) {
		flush_d(req,
				...
				"type=\"checkbox\"></th><th>Name</th><th>Date</th><th>Size</"
				"th><th>Size</th><th>Type</th><th>Mode</th><th>Actions</th></"
				"tr></thead><tbody>",
				data, flush_size);
	} else if (mode == DATA_MODE_JSON) {
		flush_d(req, "{\"path\":\"", data, flush_size);
	...
	while ((ent = readdir(dirp)) != NULL) {
		...
		total += file_handler(req, &pathbuf, len, ent->d_name, data, flush_size,
							  nitems, mode);
		++nitems;
	}
	closedir(dirp);
	if (mode == DATA_MODE_HTML)
		flush_d(req, "</tbody></table>", data, flush_size);
	else if (mode == 1) {
		strbf_putc(data, ']');
	...
	FUNC_ENTRY_ARGSD(TAG,
					 "Total %" PRIu32 " items, %llu bytes, %zu bytes sent.",
					 nitems, total, i);
done:
	IMEAS_END_ARGS(TAG, " %s done", req->uri);
	task_memory_info(__func__);
...
static esp_err_t http_resp_file_html_handler(httpd_req_t *req, const char *name,
											 strbf_t *data, size_t flush_size) {
	FUNC_ENTRY_ARGS(TAG, "%s", req->uri);
	IMEAS_START();
	flush_d(req,
			"<!DOCTYPE html><html lang=\"en\"><head><title>ESP-LOGGER ::", data,
			flush_size);
	flush_d(req, name, data, flush_size);
	flush_d(
		req,
		...
		"initial-scale=1\"></head><body><header><nav><ul>"
		"<li class=\"brand\"><span class=\"lg\">ESP-LOGGER</span></ul><ul><li "
		"class=\"home\">"
		"<a href=\"/\">Home</a></li><li class=\"files\"><a "
		"href=\"/files.html\">"
		"Files</a></li><li class=\"config\"><a "
		"href=\"/config.html\">Config</a></li>"
		"<li class=\"fwupdate\"><a href=\"/fwupdate.html\">FW Update</a></li>"
		"<li><a class=\"restart\" "
		"href=\"\">Restart</a></li></ul></nav></header>"
		"<main><div class=\"container\"><article class=\"card ",
		data, flush_size);
	flush_d(req, name, data, flush_size);
	flush_d(req, "\"><header class=\"card-header\"><div class=\"flexrow\">",
			data, flush_size);
	if (!strcmp(name, "home")) {
		flush_d(req, html_handler_str[0], data, flush_size);
	} else if (!strcmp(name, "files")) {
		flush_d(req,
				"<h2>Files</h2><div class=\"left selection hide\" "
				"hidden><button class=\"outline dl\">Download selected</button>"
				"<button class=\"outline rm\">Delete selected</button><button "
				"class=\"outline ar\">Archive selected</button></div><div "
				"class=\"right upload-file\">"
				"<label class=\"upload-file-select\"><span "
				"class=\"file-text\">Upload file</span><input type=\"file\" "
				"/></label><button class=\"outline upload-submit "
				"hide\">Upload</button>"
				"</div></div><div class=\"flexrow info\">",
				data, flush_size);
	} else if (!strcmp(name, "config")) {
		flush_d(req,
				"<h2>Configuration</h2><button class=\"right "
				"config-cmd-reset\">Reset to defaults</button>",
				data, flush_size);
	...
	if (!strcmp(name, "fwupdate")) {
		flush_d(req,
				" upload-file\"><label class=\"upload-file-select\"><span "
				"class=\"file-text\">Select firmware file</span><input "
				"type=\"file\" /></label>"
				"<button class=\"outline "
				"upload-submit\">Upload</button></div><footer "
				"class=\"card-bottom\"><span>Current firmware version: "
				"</span><span class=\"fwver-text\"></span></footer>",
				data, flush_size);
	} else {
		flush_d(req, "\">", data, flush_size);
		if (!strcmp(name, "home")) {
...
static esp_err_t embed_get_handler(httpd_req_t *req, const uint8_t *start,
								   const uint8_t *end) {
	FUNC_ENTRY_ARGS(TAG, "%s", req->uri);
	IMEAS_START();
	int ret = ESP_OK;
		...
		if (read_bytes > 0) {
			...
			if (bs) {
				...
				if (diff < 0) { // need bigger buffer...
					for (int i = be - data + diff, j = read_bytes - 1; i <= j;
						 --j) {
						data[j - diff] = data[j];
					}
				}
...
static esp_err_t try_local_file(httpd_req_t *req, size_t ulen, const char *name,
								strbf_t *data, size_t blen) {
	...
	// WLOG(TAG, "[%s] found file name %s ext %s or not.", __func__, &tmp[0], p
	// ? p : "-");
	if ((*req->uri == '/' && ulen == 1) || (tmp[0] && !strcmp(p, ".html"))) {
	index:
		if ((!tmp[0] || !strcmp(tmp, "index")) &&
			!http_resp_file_html_handler(req, "home", data, blen)) {
			goto finishing;
		} else if ((!strcmp(tmp, "404") || !strcmp(tmp, "files") ||
					!strcmp(tmp, "config") || !strcmp(tmp, "fwupdate")) &&
				   !http_resp_file_html_handler(req, &tmp[0], data, blen)) {
			goto finishing;
		}
	} else if (tmp[0] && !strcmp(tmp, "index")) {
		if ((!strcmp(p, ".css") && !css_get_handler(req)) ||
			(!strcmp(p, ".js") && !js_get_handler(req))) {
			goto finishing;
		}
...
esp_err_t add_cors(httpd_req_t *req, uint8_t api) {
	...
	if (api) {
		...
		if (strstr(uri, "files") == uri || strstr(uri, "paths") == uri) {
			err = httpd_resp_set_hdr(req, http_async_handler_strings[5],
									 "GET, POST, DELETE");
			if (err != ESP_OK) {
				ELOG(TAG, "%s", http_async_handler_status_strings[3]);
			}
...
try_again:
	...
	if (base_path_needed) {
		if (base_path_needed > 1) {
			...
			strbf_puts(pathbuf,
					   vfs_ctx.parts[vfs_ctx.gps_log_part].mount_point);
		}
	}
	if (!ret && ulen > tlen && *p == '/') {
		if (!strcmp(p + 1, "delete")) {
			ret = 1;
			p = p + 7;
		} else if (!strcmp(p + 1, "archive")) {
			ret = 2;
			p = p + 8;
		}
	}
	if (r > p) {
		if ((pathbuf->cur > pathbuf->start && *(pathbuf->cur - 1) != '/') &&
			*p != '/')
			strbf_putc(pathbuf, '/');
		s_uri_decode(pathbuf->cur, p,
					 r - p); // only p string before delete or archive
	}
	while (pathbuf->cur && *pathbuf->cur)
		++pathbuf->cur;
	*pathbuf->cur = 0;
	FUNC_ENTRY_ARGSD(TAG, "filepath:%s p:%s r:%s len:%zu", pathbuf->start, p, r,
					 r - p);
...
esp_err_t api_handler(httpd_req_t *req) {
	...
	size_t ulen = strlen(req->uri), tlen = 8,
		   flush_size = SCRATCH_BUFSIZE - 256, msglen = 0;
	...
	if (strstr(uri, "config") == uri) {
		...
		case 9:
			...
			if (err < 8) {
				DLOG(TAG, "Going to %s file: %s, uri: %s", p, pathbuf.start,
					 req->uri);
				if (ret == 2)
					err = archive_file(
						req, pathbuf.start,
...
static esp_err_t system_format_handler(httpd_req_t *req, strbf_t *data) {
	...
	if (len == 0 || len >= 64) {
		httpd_resp_set_status(req, HTTPD_400);
		http_send_json_msg(
			req, len == 0 ? "Missing mountpoint" : "Mountpoint too long",
			len == 0 ? 17 : 19, 0, 0, 0);
		return ESP_FAIL;
	}
...
esp_err_t get_handler(httpd_req_t *req) {
	...
	if (strstr(req->uri, "/files") == req->uri) {
		...
		if (err == 3) {
			directory_handler(req, pathbuf.start, 0, 2, &buf,
							  SCRATCH_BUFSIZE - 128);
			goto finishing;
		} else
			goto get_file;
...
static esp_err_t archive_file_cb(httpd_req_t *req, const char *fname) {
	FUNC_ENTRY_ARGS(TAG, "%s", fname);
	return archive_file(req, fname,
						vfs_ctx.parts[vfs_ctx.gps_log_part].mount_point);
}

static esp_err_t bulk_manage_files(httpd_req_t *req, char *fname, size_t flen,
								   strbf_t *data, manage_file_cb_t cb,
								   const char *action_name) {
	if (!data)
		return ESP_ERR_INVALID_ARG;
	FUNC_ENTRY_ARGS(TAG, "%s %s", data->start,
		...
		while (p && (!e || p < e)) {
			...
			if (r) {
				if (((fbuf.cur - fbuf.start) + (r - p) + 1) >= flen) {
					ret = http_send_json_msg(req, "Filename too long.", 18, 2,
											 0, 0);
					goto done;
				}
...
esp_err_t post_handler(httpd_req_t *req) {
	...
	if (mpb) {
		...
		if (mpb) {
			...
			TLOG(TAG, "boundary found '%s' size '%" PRIu16 "'", boundary,
				 boundarylen);
		}
	}
	...
	while (total_len > 0) {
		...
		now = get_millis();
#endif
		if ((recieved = httpd_req_recv(req, buf, MIN(total_len, buflen))) <=
			0) {
		...
		if (is_multipart) {
			...
			if (recieved < buflen && recieved >= total_len) {
				FUNC_ENTRY_ARGSD(TAG,
								 "all data recieved bl:%" PRIu16 " rc:%d tl:%d",
								 buflen, recieved, total_len);
				*(buf + recieved) = 0;
			}
			mpb = mpb0 = buf;
			FIND_B
			...
			open_mpart:
				...
				FUNC_ENTRY_ARGS(
					TAG, "found multipart file begin, fname: '%s' len: '%d'",
					fname, fnamelen);
				parts[mpart_num].end_mark = (buf + recieved);
				// printf("[%s] start mpb(001): '%s' \n", __FUNCTION__, mpb);
				while (mpb && mpb < parts[mpart_num].end_mark &&
					   !(is_crln_safe(mpb) && is_crln_safe(mpb + 2))) {
					++mpb;
				}
				// printf("[%s] start mpb(01): '%s' \n", __FUNCTION__, mpb);
				while (
					is_cr_o_ln(mpb)) { // pointer back to first char after CRLF
					++mpb;
				}
			...
			if (mpb && mpart_open) { // mpart end
				...
				GOMP_S // move back to the beginning of second boundary
					while (is_cr_o_ln(
						mpb)) { // pointer back to last char before CRLF
					--mpb;
				}
				...
				mpb0 = buf + recieved;
				while (
					is_cr_o_ln(mpb)) { // pointer back to last char before CRLF
					++mpb;
				}
				while (mpb && mpb < mpb0 &&
					   !is_crln_safe(
						   mpb)) // move cursor to newline at end of boundary
					++mpb;
				if (is_cr_o_ln(mpb)) // move to last not crln char to test end
			...
			if (u_mode == 1 && parts[0].end_mark) {
				err = ota_write((uint8_t *)parts[0].start_mark,
								parts[0].end_mark - parts[0].start_mark);
				if (err != ESP_OK) {
				...
				if (!fname[0]) // no file name, no upload
					...
					if ((p = strstr(req->uri, "/files/")) && p[7]) {
						...
						if (e - p > 128) {
							WLOG(TAG,
								 "Too long path, max allowed 128 bytes: %s",
								 pbuf.start);
						}
					} else {
						strbf_put_path(
	...
	DLOG(TAG, "Post request saved %" PRIu32 " bytes.", l);
	if (!is_multipart) {
		if (!data.start || data.cur == data.start) {
			goto toerr;
		}
	...
	/* rest_server_context_t *rest_context = (rest_server_context_t
	 *)req->user_ctx; assert(rest_context); */
	...
	if (api) {
		...
		if (strstr(mpb, "files/delete") == mpb) {
			if (bulk_manage_files(req, fname, 64, &data, delete_file_cb,
								  "delete") != 0) {
		...
		if (strstr(mpb, "files/archive") == mpb) {
			if (bulk_manage_files(req, fname, 64, &data, archive_file_cb,
								  "archive") != 0) {
			...
			if (r && *r) {
				...
				// Bulk JSON config updates - use reasonable timeout to avoid
				// priority inversion blocking
				if (!config_lock(
						1000)) { // 1 second timeout instead of portMAX_DELAY
			...
			if (save_result == 0) {
				...
				if (r && config_manager_get_item_by_name(r, &respsb) >= 0) {
					http_send_json_msg(req, "Saved", 5, 0, respsb.start,
									   respsb.cur - respsb.start);
				} else {
...
void start_async_req_workers(void) {
	...
	for (int i = 0; i < worker_num; i++) {
		...
		bool success = xTaskCreatePinnedToCore(
			async_req_worker_task, "async_req_worker",
